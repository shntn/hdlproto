...
from hdlproto.signal_array import RegArray
...
# =================================================================
def test_reg_array_init_scalar(ctx):
    """
    ...
    arr = RegArray(count=count, width=width, init=init_val)
    ...
    for i, r in enumerate(arr):
        r._set_context(f"r{i}", None, ctx)
        assert isinstance(r, Reg)
        assert r._get_width() == width
        assert r.r == init_val


def test_reg_array_init_list_padding(ctx):
    """
    _make_initialized_data (リスト分岐) のテスト。
    要素数が足りない場合に 0 でパディングされるか確認。
    """
    # 3つ要求するが、初期値は2つだけ提供
    arr = RegArray(count=3, width=8, init=[0xAA, 0xBB])
    ...
    assert arr[0].r == 0xAA
    assert arr[1].r == 0xBB
    assert arr[2].r == 0x00  # パディングされた値


def test_reg_array_init_tuple(ctx):
    """
    _make_initialized_data (タプル分岐) のテスト。
    タプルで渡してもリスト同様に処理されるか。
    """
    arr = RegArray(count=3, width=8, init=(0x1, 0x2))
...
def test_reg_array_init_default():
    """
    init 引数を省略した場合のデフォルト動作 (すべて0) を確認。
    """
    arr = RegArray(count=2, width=4)
...
# =================================================================
def test_reg_array_getitem_index(ctx):
    """
    _SignalArray.__getitem__ のインデックスアクセスのテスト。
    """
    arr = RegArray(count=3, width=8, init=[10, 20, 30])
...
def test_reg_array_getitem_tuple_access(ctx):
    """
    _SignalArray.__getitem__ のタプルアクセス (arr[index, slice]) のテスト。
    これにより _SignalArray 内の `if isinstance(key, tuple):` 分岐をカバーする。
    """
    arr = RegArray(count=2, width=8, init=[0x5A, 0xC3])
    arr[0]._set_context("r", None, ctx)

    # arr[0, 7:4] -> 0番目のRegの上位4ビットを取得
    # 期待値: 0101 -> 0x5
    slice_val = arr[0, 7:4]
    assert slice_val == 0x5

    # arr[1, 7] -> 1番目のRegの 7ビット目を取得
    # 期待値: 1
    bit_val = arr[1, 7]
    assert bit_val == 1


def test_reg_array_iter(ctx):
    """
    __iter__ のテスト。リスト内包表記などで回せるか。
    """
    init_vals = [1, 2, 3]
    arr = RegArray(count=3, width=8, init=init_vals)

    extracted_vals = [r.r for r in arr]
...
# 3. RegArray 固有メソッド
# =================================================================
def test_reg_array_set_array(ctx):
    """
    _set_array メソッドのテスト。
    内部の _base (_SignalArray) が差し替わるか確認。
    """
    arr = RegArray(count=1, width=8, init=10)
    old_reg = arr[0]

    # 新しいリストで上書き
    new_regs = [Reg(width=8, init=99), Reg(width=8, init=100)]
    arr._set_array(new_regs)
...
def test_reg_array_write_behavior(ctx):
    """
    RegArrayの要素に対して書き込みを行い、シミュレーションコンテキストに記録されるか確認。
    (Regクラスとの結合確認)
    """
    arr = RegArray(count=1, width=8)
    r = arr[0]
    r._set_context("r0", None, ctx)
