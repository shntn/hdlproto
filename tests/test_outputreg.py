import pytest
import pickle
from hdlproto.signal import Wire, Reg, OutputReg
...
def setup_nested_output_reg(ctx, width=8, init=0):
    """
    テスト用ヘルパー: OutputReg(top) -> OutputReg(mid) -> Reg(root) を構築して返す。
    戻り値: (root_reg, top_output_reg)
    """
    # 1. 実体 (Root)
    root = Reg(width=width, init=init)
    root._set_context("root_reg", None, ctx)
...
# =================================================================
def test_output_reg_pass_through(ctx):
    """
    OutputRegへの書き込みが、ターゲットとなるRegへ
    正しく反映されることを確認する。
    """
    # ターゲット作成
    target = Reg(width=8, init=0x00)
    target._set_context("target_reg", None, ctx)

    # OutputReg作成
    out = OutputReg(target)
    out._set_context("out_port", None, ctx)

    # 1. 初期値の確認
    assert out.r == 0x00, "Should reflect target initial value"

    # 2. OutputRegの値を変更 (out -> target)
    out.r = 0xFF
    # OutputRegへの書き込みは即時反映ではなくコミット待ちになるため、実体をコミット
    target._commit()

    # 3. ターゲット経由で確認
    assert out.r == 0xFF, "Target should reflect updated OutputReg value"
    assert out._get_width() == 8, "Should reflect target width"

def test_output_reg_slice_write(ctx):
    """
    OutputRegに対するスライス書き込みが、ターゲットへのスライス操作として
    正しく委譲されるか確認する。
    """
    target = Reg(width=8, init=0xCA)
...
# =================================================================
def test_output_reg_rejects_wire(ctx):
    """
    OutputReg は Wire をラップできない仕様（TypeError）であることを確認する。
    """
    w = Wire(width=8)

    # OutputWireを使うべき場面でOutputRegを使った場合のエラー
    with pytest.raises(TypeError, match="OutputReg cannot wrap a Wire"):
        _ = OutputReg(w)


def test_output_reg_invalid_write_type(ctx):
    """
    OutputRegへの書き込み時に、不正な型（文字列など）が弾かれるか確認する。
    """
    target = Reg(width=8)
    out = OutputReg(target)
    # 書き込みを行う前に必ずコンテキストを設定する
    out._set_context("out_port", None, ctx)
...
# -----------------------------------------------
def test_output_reg_snapshot_epsilon_only(ctx):
    """
    OutputReg._snapshot_epsilon() を呼んだ時、
    epsilonだけが更新され、deltaやcycleには影響しないことを確認する。
    """
    # 1. 初期化 (値: 0)
    target = Reg(width=1, init=0)
    ...
    # 3. OutputReg経由で Epsilon だけスナップショットを撮る
...
# -----------------------------------------------
def test_output_reg_snapshot_delta_only(ctx):
    """
    OutputReg._snapshot_delta() を呼んだ時、
...
# -----------------------------------------------
def test_output_reg_snapshot_cycle_only(ctx):
    """
    OutputReg._snapshot_cycle() を呼んだ時、
...
# -----------------------------------------------
def test_output_reg_edge_detection_rising(ctx):
    """
    OutputReg 経由で、立ち上がりエッジ (0 -> 1) が正しく判定されるか確認する。
    """
    # 1. 初期化 (0)
    target = Reg(width=1, init=0)
    target._set_context("t", None, ctx)
...
def test_output_reg_edge_detection_falling(ctx):
    """
    OutputReg 経由で、立ち下がりエッジ (1 -> 0) が正しく判定されるか確認する。
    """
    # 1. 初期化 (1)
    target = Reg(width=1, init=1)
    target._set_context("t", None, ctx)
    out = OutputReg(target)
...
# =================================================================
def test_output_reg_internal_attributes(ctx):
    """
    ...
    # _is_reg プロパティ (OutputRegはRegなのでTrue)
    assert out._is_reg is True

    # _get_signal() は「皮を剥いた」中身（ターゲットの実体）を返す仕様
    assert out._get_signal() is target


def test_output_reg_name_formatting(ctx):
    """
    OutputRegの名前解決が 'port_name(target_name)' 形式になるか確認する。
    """
    target = Reg()
    target._set_context("internal_reg", None, ctx)

    out = OutputReg(target)
    out._set_context("port_a", None, ctx)

    assert out._get_name() == "port_a(internal_reg)"


# =================================================================
# OutputReg ネスト構成 (Chain Delegation) の統合テスト
# =================================================================

def test_nested_pass_through_and_slice(ctx):
    """
    ネストされたOutputRegを通して、値の書き込みとスライス操作が
    最下層のRegまで正しく届くか確認する。
    """
    root, top = setup_nested_output_reg(ctx, width=8, init=0x55)

    # 1. 初期値の確認
    assert top.r == 0x55, "Should reflect root value"
    assert top._get_width() == 8, "Should reflect root width"

    # 2. Topへの書き込み (InputRegとは逆: Top -> Root)
    top.r = 0xCA  # 1100_1010
    top._commit()

    # 3. Rootへの反映確認
    assert top.r == 0xCA, "Should reflect updated top value to top"
    assert root.r == 0xCA, "Should reflect updated top value to root"

    # 4. スライス書き込みの委譲確認
    # 上位4ビットに 0x0 を書く (元はC) -> 0x0A になるはず
    top[7:4] = 0x0
    root._commit()
    assert top.r == 0x0A, "Slice write delegation failed (upper)"
    assert root.r == 0x0A, "Slice write delegation failed (upper)"

    # 下位4ビットに 0xF を書く (元はA) -> 0x0F になるはず
    top[3:0] = 0xF
    root._commit()
    assert top.r == 0x0F, "Slice write delegation failed (lower)"
    assert root.r == 0x0F, "Slice write delegation failed (lower)"
...
def test_nested_constraints_and_internal_api(ctx):
    ...
    """
    root, top = setup_nested_output_reg(ctx)
    ...
    # 2. _is_reg の委譲 (OutputRegなのでTrue)
    assert top._is_reg is True, "Nested OutputReg should report _is_reg as True"

    # 3. _get_signal() の再帰的解決
    # OutputReg -> OutputReg -> Reg なので、皮を剥いて最後の Reg が返るべき
    assert top._get_signal() is root, \
        "_get_signal() should verify recursively and return the root Reg"


def test_nested_snapshot_isolation(ctx):
    """
    ネストされたOutputReg経由でスナップショットを撮った際、
    関数が入れ替わらず、正しく最下層まで命令が届くか確認する。
    """
    root, top = setup_nested_output_reg(ctx, width=1, init=0)
...
def test_nested_edge_detection(ctx):
    """
    ネストされたOutputReg経由で、エッジ検出(POS/NEG)が正しく判定されるか確認する。
    """
    root, top = setup_nested_output_reg(ctx, width=1, init=0)
...
# __getnewargs__
# =================================================================
def test_output_reg_getnewargs_direct_call():
    """
...
def test_output_reg_pickle_roundtrip(ctx):
    """
    実際に pickle (シリアライズ) して復元できるかを確認する統合テスト。
    """
    # ターゲット (値: 0xAA)
    target = Reg(width=8, init=0xAA)
    target._set_context("t", None, ctx)
    ...
    # 3. 検証
    assert isinstance(restored_out, OutputReg), "Restored object should be OutputReg"
    assert restored_out.r == 0xAA, "Restored object should retain the value"

    # pickleの仕様上、実体は別物になる
    assert restored_out._get_signal() is not target, "Pickle creates a copy of the target"
    assert restored_out._get_signal().r == 0xAA, "Copied target should have the same value"